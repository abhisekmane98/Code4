...
void sum_sal_array(void)
{
	...
	while(1)
	{
		printf("Enter 1 for sum of original array\nEnter 2 for sum of modified/reallocated array");
		scanf("%d",&x);
		if(x==1)
		{
			for(i=0;i<=n-1;i++)	
		   ...
		   {
		        printf("You enterd wrong entry so please ");
		        continue;
		   }
   }
}
...
void max_array(void)
{
	...
	while(1)
	{
		...
		if(x==1)
		{
		   ...
		   {
		        printf("You enterd wrong entry so please ");
		        continue;              // not necessary to write continue think....????????.......
		   }
   }
}
